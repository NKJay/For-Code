...
class AndroidViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
...
//        })
        ...
        let f = NSFetchRequest(entityName: "AndroidNews")
    ...
    func loadData(URL:String){
        ...
        afmanager.GET(loadUrl, parameters: nil, success: { (AFHTTPRequestOperation, resp:AnyObject) -> Void in
            ...
            for each in results{
                let item = NewsItem()
                let row = NSEntityDescription.insertNewObjectForEntityForName("AndroidNews", inManagedObjectContext: self.context)
...
//                self.newsTableView.mj_footer.endRefreshing()
    ...
    //    加载更多数据
    func loadMoreData(){
        URL = "http://gank.avosapps.com/api/data/Android/10/"+String(i++)
        let afmanager = AFHTTPRequestOperationManager()
